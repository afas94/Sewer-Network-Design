...
public class Main {
	...
	public static void main(String[] args) {
		...
		try {
			manager.setFile("CHICO.txt");
			int iter = 0;
			while (iter <=1) {
				System.out.print(iter + "\t");
				manager.initialize(1,nManning, elevationChange,minDepth, maxDepth, typeCost);
				iter++;
				
				System.out.print(iter + "\t");
				manager.execute(1, nManning, elevationChange,minDepth, maxDepth, typeCost);
				iter++;
			}
