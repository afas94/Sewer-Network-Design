...
//	public static final String mm = "mm";
	 ...
	 * @param inputData 
	 */
	public Design_GraphBuilder(DataHandler data, double nManning,int nCostType,double precision) {
		
		dh=data;
		
		hydraulics = new Design_Hydraulics(1.14*Math.pow(10, -6), nManning, nCostType);
...
//												System.out.println("DNodeUp: "+dNodeUp+" DNodeDown: "+dNodeDown);
												int inicios= contarInicios(loNodeUp);
												cost=hydraulics.costo(diameterDown, distancePythagoras, depthUp, depthDown,inicios);
...
//				System.out.println(pendientes);
	...
	private int contarInicios(Layout_Node loNodeUp) {
		ArrayList<Layout_Section> secciones = loNodeUp.layoutSections_in;
		int inicios= 0;
		for(int i=0;i<secciones.size();i++){
			if(secciones.get(i).loNode_type.equals("I")){
				inicios++;
			}
		}
